...
20, 15, 10, and 17 metres, and Mirko raises his sawblade to 15 metres, the remaining tree heights after cutting will be 15, 15, 10, and 15 metres, 
...
Input
The first line of input contains two space-separated positive integers, N (the number of trees, 
1 ≤ N ≤ 1 000 000) and M (Mirko‟s required wood amount, 1 ≤ M ≤ 2 000 000 000).
...
bool isPossibleToPlaceBlade(long long int trees[], int n, int length, int maxHeight){
    long long int tempHeight = 0;
    for(int i=0; i<n; i++){
        if(trees[i]-maxHeight > 0)
            tempHeight += (trees[i]-maxHeight);
    }
    if(tempHeight < length) return false;
    return true;
}

int getMaximumHeightForSawBlade(long long int trees[], int n, int length){
    long long int maxTree = INT_MIN;
    for(int i=0; i<n; i++){
        if(trees[i] > maxTree)
            maxTree = trees[i];
    }
    ...
    long long int s = 0;
    long long int e = maxTree;
    long long int ans = -1;

    while(s <= e){
        long long int mid = s + (e-s)/2;
        if(isPossibleToPlaceBlade(trees, n, length, mid)){
            ans = max(ans, mid);
            s = mid+1;
        }else{
            e = mid-1;
...
int main(){
    ...
    cout<<getMaximumHeightForSawBlade(trees, n, length)<<endl;
